...
public class FahrtBewerten implements Closeable {
	...
	// checkt ob der User die Fahrt schon bewertet hat

	public boolean RatingCheck(int benutzer, int fid) {
		boolean rated = false;

		String QueryCheck = "select * FROM schreiben s where s.benutzer = ? AND s.Fahrt = ? ";
		try {
	...
	public void fahrtBewerten(String BewertungsText, Timestamp erstellungsdatumnew, int Bewertungsrate, int benutzer,
			int FahrtTogetBewertet1) {

		String QueryBewertung = "INSERT INTO bewertung (textnachricht, erstellungsdatum, rating) VALUES	(?, ?, ?)";
		// Lesen der Bewertung ID von der hinzugefügte Bewertung
		String QueryRead = "select beid FROM bewertung ORDER BY ERSTELLUNGSDATUM DESC FETCH FIRST 1 ROW ONLY ";
		// Der Benutzer, der diese Bewertung schreibt, speichern mit Hilfe von BEID, die
		// gelesen wurde
		String QuerySchreiben = "INSERT INTO schreiben (benutzer, fahrt, bewertung) VALUES (?, ?, ?)";
