...
public class FahrtDetail implements Closeable {
	...
	public void detail(int fid) throws RuntimeException {
		...
		String QuerySelected = "select email,startort,status,zielort,reser,fahrtkosten, Beschreibung,fahrtdatumzeit,maxplaetze from benutzer b,fahrt f LEFT OUTER JOIN (select fahrt,sum(r.ANZPLAETZE)as reser FROM reservieren r Group by r.Fahrt)a on a.fahrt = f.fid where b.bid = f.anbieter and fid = ? ";
		String QueryDurchschnitt = "select avg(rating) as durchschnitt FROM FAHRT f INNER JOIN(select * from bewertung b INNER JOIN schreiben r ON  b.beid = r.bewertung)a on a.fahrt = f.fid where fid = ? ";
		String QueryRatings = "select fahrt,email,rating, textnachricht, benutzer from benutzer be INNER JOIN(select * from schreiben s, bewertung b where b.beid= s.bewertung)o ON be.bid = o.benutzer AND o.fahrt = ? order by erstellungsdatum desc ";
		try {
			...
			try (PreparedStatement pre = connection.prepareStatement(QuerySelected)) {
				...
				while (FD.next()) {
					...
					if (frei >= 0) {
						...
						FahrtD Fobject = new FahrtD(anbieter, Fahrdatumunduhrzeit, von, nach, frei, fahrtkosten, status,
								beschreibung);
